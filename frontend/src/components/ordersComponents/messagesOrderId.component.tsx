...
const MessagesOrderIdComponent = () => {
  ...
  const { messagesOrderId, findOneOrder, findOneOrderError, isEditOrder, openedMessageOrderId } =
    useAppSelector((state) => state.orderStore);
  const messageClose = useRef<HTMLDivElement | null>(null);
  const dispatch = useAppDispatch();

  useEffect(() => {
    if (openedMessageOrderId) {
      dispatch(orderAction.loadFindOneOrder(openedMessageOrderId));
      dispatch(orderAction.loadMessagesOrderId(openedMessageOrderId));
    }
  }, [dispatch, openedMessageOrderId]);
  ...
  return(
    <div ref={messageClose}>
      { findOneOrderError ? (
        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '4vh', background: 'rgba(209, 115, 115, 0.46)', borderRadius: '10px'}}>
          <p style={{margin: 0, color: '#6e0707'}}>{findOneOrderError}</p>
        </div>
      ) : (
        <div className={'divMainLayout__outlet__ordersAllPage__ordersTableComponent__table__tbody__messagesOrderIdComponent'}>
          <div>
          ...
          <div
            className={'divMainLayout__outlet__ordersAllPage__ordersTableComponent__table__tbody__messagesOrderIdComponent__messages'}>

            <div
              className={'divMainLayout__outlet__ordersAllPage__ordersTableComponent__table__tbody__messagesOrderIdComponent__messages__data'}>
              {messagesOrderId.map(value =>
                <div
                  className={'divMainLayout__outlet__ordersAllPage__ordersTableComponent__table__tbody__messagesOrderIdComponent__messages__data__divMap'}
                  key={value.id}>
                  <div
                    className={'divMainLayout__outlet__ordersAllPage__ordersTableComponent__table__tbody__messagesOrderIdComponent__messages__data__divMap__text'}>
                    <p><b>comment:</b> {value.messages}</p></div>
                  <div
                    className={'divMainLayout__outlet__ordersAllPage__ordersTableComponent__table__tbody__messagesOrderIdComponent__messages__data__managerDate'}>
                    <p
                      className={'divMainLayout__outlet__ordersAllPage__ordersTableComponent__table__tbody__messagesOrderIdComponent__messages__data__managerDate__p'}>{value.manager}</p>
                    <p
                      className={'divMainLayout__outlet__ordersAllPage__ordersTableComponent__table__tbody__messagesOrderIdComponent__messages__data__managerDate__p'}>{value.created_at ? dayjs.utc(value.created_at).local().format('MMMM D, YYYY HH:mm:ss') : ''}</p>
                  </div>
                </div>)}
            </div>

            <form
              className={'divMainLayout__outlet__ordersAllPage__ordersTableComponent__table__tbody__messagesOrderIdComponent__messages__form'}
