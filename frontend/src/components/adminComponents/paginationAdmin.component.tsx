...
const PaginationAdminComponent = () => {
  ...
  const renderPage = ()=> {
    if(totalPages <= 7){
      return (
        ...
        Array.from({ length: totalPages }, (_, index) => (
          <button className={'divMainLayout__ordersAllPage__paginationComponent__button'} key={index + 1} onClick={() => handlePageClick(index + 1)} disabled={currentPage === index + 1}>
            {index + 1}
          </button>))
          }
        </div>)
    ...
    if(currentPage <= 7){
      ...
      <div>
        ...
        <button className={'divMainLayout__ordersAllPage__paginationComponent__button'} key={totalPages} onClick={() => handlePageClick(totalPages)} disabled={currentPage === totalPages}>
          {totalPages}
        </button>
      </div>)
  } else {
      ...
      startAdminPage = Math.max(startAdminPage, 2);
          ...
          const middlePages = Array.from({length: endPage - startAdminPage + 1}, (_,index) => {
            return (
            <button className={'divMainLayout__ordersAllPage__paginationComponent__button'} key={startAdminPage + index} onClick={() => handlePageClick(startAdminPage + index)} disabled={currentPage === startAdminPage + index}>
  ...
  return(
    <div className={'divMainLayout__ordersAllPage__paginationComponent'}>
      {currentPage > 1 && (
